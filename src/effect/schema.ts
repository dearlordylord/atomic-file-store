...
import { ConflictExhausted, ContentsInvalidError, ReadError, WriteError } from "./errors.js"
import type { StateFilePath } from "./path.js"
import type { StateFileLocks } from "./locks.js"
...
> => {
  ...
  const decode = (contents: string): Effect.Effect<A, ContentsInvalidError, RSchema> =>
    Schema.decodeUnknown(jsonSchema)(contents).pipe(Effect.mapError(() => new ContentsInvalidError(path)))
  const encode = (value: A): Effect.Effect<string, ContentsInvalidError, RSchema> =>
    Schema.encode(jsonSchema)(value).pipe(Effect.mapError(() => new ContentsInvalidError(path)))
  ...
  ): Effect.Effect<CarriedModifyOutcome<A, C>, ContentsInvalidError, RSchema> => {
