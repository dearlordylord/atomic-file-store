 ...
 * how a background keepalive once reverted fresh interactive logins.
 *
...
import { AbsentError, ConflictExhausted, ReadError, WriteError } from "./errors.js"
import { StateFileLocks } from "./locks.js"
import type { StateFilePath } from "./path.js"
...
const readOptionalRaw = (path: string): Effect.Effect<string | undefined, ReadError> =>
  Effect.tryPromise({
    try: () =>
 ...
 * fails with `AbsentError`, which a caller that treats "no state yet" as
...
export const read = (path: StateFilePath): Effect.Effect<string, AbsentError | ReadError> =>
  readOptionalRaw(path).pipe(
    Effect.flatMap((contents) =>
      contents === undefined ? Effect.fail(new AbsentError(path)) : Effect.succeed(contents)
    )
  )
 ...
 */
const fileSystemCall = <A>(path: string, call: () => Promise<A>): Effect.Effect<A, WriteError> =>
 ...
 */
type Landing = (temporaryPath: string) => Effect.Effect<boolean, WriteError>
...
const replaceRawAtomic = (path: string, contents: string): Effect.Effect<void, WriteError> =>
 ...
 * winner.
 */
const createRawExclusive = (path: string, contents: string): Effect.Effect<boolean, WriteError> =>
  writeThroughTemporary(path, contents, (temporaryPath) =>
    fileSystemCall(path, () =>
...
): Effect.Effect<CarryOutcome<W, K>, WriteError | ReadError | ConflictSignal<W>> =>
  Effect.gen(function* () {
...
): Effect.Effect<CarryOutcome<W, K>, ReadError | WriteError | ConflictSignal<W> | E, R> =>
  Effect.gen(function* () {
...
export const modifyCarrying = <W, K, E, R>(
  ...
  CarryOutcome<W, K>,
  ReadError | WriteError | ConflictExhausted | E,
  R | StateFileLocks
> => {
  ...
  return attempt.pipe(
    ...
    Effect.catchIf(isConflictSignal, () => Effect.fail(new ConflictExhausted(path)))
  )
}
