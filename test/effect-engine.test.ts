...
const makeTempDir = Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "atomic-file-store-")))
...
describe("read", () => {
  ...
  itLocks("fails with AbsentError when the file does not exist", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir
      const file = stateFile(dir)

      const error: AbsentError | ReadError = yield* read(file).pipe(Effect.flip)

      // absence has its own tag: "no state yet" is not "cannot read this file"
      expect(error._tag).toBe("AbsentError")
      expect(error.message).toContain(file)
    })
  )

  itLocks("fails with ReadError when the path cannot be read", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir

      const error: AbsentError | ReadError = yield* read(statePath(dir)).pipe(Effect.flip)
...
describe("modify: creation", () => {
  ...
  itLocks("fails with WriteError when creation cannot land", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir
...
describe("modify: failures", () => {
  ...
  itLocks("fails with WriteError when the durable write cannot complete", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir
      ...
      if (error._tag !== "WriteError") {
        throw new Error(`expected WriteError, got ${error._tag}`)
      }

      const writeFailure: WriteError = error
...
describe("parseStateFilePath", () => {
  ...
  itLocks("rejects a relative path, which would name different files in different processes", () =>
    Effect.gen(function* () {
      const error: PathInvalidError = yield* parseStateFilePath("state.json").pipe(Effect.flip)
  ...
  itLocks("rejects a blank path and a non-string", () =>
    Effect.gen(function* () {
      expect((yield* parseStateFilePath("   ").pipe(Effect.flip))._tag).toBe("PathInvalidError")
      expect((yield* parseStateFilePath(42).pipe(Effect.flip))._tag).toBe("PathInvalidError")
    })
  )
})
