...
const makeTempDir = Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "atomic-file-store-schema-")))
...
const expectContentsInvalid = (error: ContentsInvalidError | ConflictExhausted): ContentsInvalidError => {
  if (error._tag !== "ContentsInvalidError") {
    throw new Error(`expected ContentsInvalidError, got ${error._tag}`)
  }
...
describe("modifySchema", () => {
  ...
  itLocks("fails with ContentsInvalidError when the file is not valid JSON", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir
      const file = stateFile(dir)
  ...
  itLocks("fails with ContentsInvalidError when the JSON does not match the schema", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir
      const file = stateFile(dir)
  ...
  itLocks("fails with ContentsInvalidError when the transformed value cannot be encoded", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir
      const file = stateFile(dir)
  ...
  itLocks("fails with ContentsInvalidError when the adopted contents do not match the schema", () =>
    Effect.gen(function* () {
      const dir = yield* makeTempDir
      const file = stateFile(dir)
      yield* writeRaw(file, JSON.stringify({ lineage: "S0", rotations: 0 } satisfies State))
